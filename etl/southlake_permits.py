"""
Southlake Building Permit data extractor.
...
from config import SOUTHLAKE_ENERGOV_URL
...
def fetch_southlake_permits_since(since_date: str) -> list[dict]:
    """
    Fetch Southlake building permit records from EnerGov API.
    ...
    """
    if not SOUTHLAKE_ENERGOV_URL:
        print("[Southlake] EnerGov URL not configured.")
        return []

    print(f"[Southlake] Fetching permits since {since_date}...")

    try:
        since_dt = datetime.strptime(since_date, "%Y-%m-%d")
    except ValueError:
        print(f"[Southlake] Invalid date format: {since_date}")
        return []

    # EnerGov API endpoints to try
    base_url = SOUTHLAKE_ENERGOV_URL.rstrip('/')
    search_endpoints = [
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if raw_records:  # Found working endpoint
                    print(f"[Southlake] Found {len(records)} restaurant/bar-related permits")
                    return records
    ...
    print("[Southlake] Could not connect to EnerGov API. Check endpoint configuration.")
    return []


def to_source_events(rows: list[dict]) -> list[dict]:
    """
    Map raw Southlake permit rows to normalized source_events dicts.
    """
    events = []
    ...
    for row in rows:
        ...
        event = {
            "source_system": "SOUTHLAKE_PERMIT",
